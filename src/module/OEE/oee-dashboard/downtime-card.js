...
function DowntimeCard({ startDate, endDate, assetId, shiftName }) {
  const formattedStartDate = moment(startDate).format("DD MMM, hh:mm A");
  const formattedEndDate = moment(endDate).format("DD MMM, hh:mm A");
  ...
  const options = {
    ...
    // title: {
    // text: ` ${shiftName === undefined ? "No Shift" : shiftName},${moment(
    //   startDate
    // ).format("DD MMM, hh:mm:ss A")} , ${moment(endDate).format(
    //   "DD MMM, hh:mm:ss A"
    // )}`,
    //   align: "left",
    // style: {
