...
import SchedulerService from "../../../services/quality-inspection/scheduler-service";
...
class Scheduler extends FilterFunctions {
    ...
    const content = (
      ...
      >
        ...
        <Space style={{ float: "right" }}>
          {event.executionId && (
            <Link to={`/qi/checklist-execution/update/${event.executionId}`}>
              <Button
                type="primary"
                disabled={
                  this.state.disabled ||
  ...
  handleData(data) {
    ...
    return data?.map((e) => {
      // console.log(this.checkListIdService, "checks");
      ...
      // console.log("stscnt", checkStatusCount);

      let scheduleStatus = e.checkListExecution?.status;
      let backgroundColor, color;
      // console.log("sts", scheduleStatus);
      if (scheduleStatus) {
  ...
  handleSelectEvent = ({ end, id }, editData) => {
    const currentDate = new Date();
    const eventDate = new Date(end);
    this.setState({ scheduleDate: currentDate.toISOString() });
    if (eventDate <= currentDate) {
  ...
  render() {
    ...
    // console.log("this", this.state.row);
