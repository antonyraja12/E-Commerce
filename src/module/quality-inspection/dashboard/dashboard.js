...
import DashboardService from "../../../services/quality-inspection/dashboard-service";
...
import MtbfService from "../../../services/quality-inspection/mtbf-service";
import MttrService from "../../../services/quality-inspection/mttr-service";
...
class MainDashboard extends FilterFunctions {
  ...
  handleSelectAllChange = (e) => {
    const { userList, filteredUserList } = this.state;
    const allUserIds = [...userList, ...filteredUserList].map(
      (user) => user.value
    );
  ...
  handleUserCheckboxChange = (checkedValues) => {
    const { userList, filteredUserList } = this.state;
    const allUserIds = [...userList, ...filteredUserList].map(
      (user) => user.value
    );
    const isAllSelected = checkedValues.length === allUserIds.length;

    this.setState({
      selectedUserIds: checkedValues,
      selectedFields: {
        ...this.state.selectedFields,
        userIds: isAllSelected ? allUserIds : checkedValues,
      },
    });
  };
                ...
                <Row gutter={[10, 10]}>
                  {this.state.ticketStatus?.map((e, i) => {
                    ...
                    return (
                      <Col flex={1}>
                        <Link
                          to={`../resolution-work-order?status=${index}&startDate=${startDateValue}&endDate=${endDateValue}`}
                        >
                          <TileCardOne
                            label={e.x}
                            value={e.y}
              ...
              <Col sm={24} xs={24} md={24} lg={12}>
                <Card size="small">
                  <BarChart2
                    ...
                    title="QI Checklist Execution Status"
