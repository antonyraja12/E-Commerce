...
class UserAccess extends Component {
	...
	componentDidMount() {
		this.fetchDataAndProcessMenuList()
			.then(this.handleFetchedData)
			.catch((error) => {
				console.error(
					"Error fetching and processing menu list:",
					error
				);
			});
	}

	fetchDataAndProcessMenuList = () => {
		return Promise.all([
	...
	handleFetchedData = ([response1, response2, response3, response4]) => {
		const mapFeatureNamesToMenuItems = (menuItems, featureList) => {
			return menuItems.map((menuItem) => {
				if (menuItem.userAccessFeatureMappings && featureList) {
					menuItem.userAccessFeatureMappings =
						menuItem.userAccessFeatureMappings
							.map((mappingId) => {
								const feature = featureList.find(
									(feature) =>
										feature.userAccessFeatureId ===
										mappingId
								);
								return feature
									? {
											userAccessFeatureMappingId:
												mappingId,
											...
											userAccessFeature: {
												...
												featureName:
													feature.featureName,
												status: true, // Ensure status is always true
											},
									  }
									: null;
							})
				...
				if (menuItem.children) {
					menuItem.children = mapFeatureNamesToMenuItems(
						menuItem.children,
						featureList
					);
				}
		...
		const flattenMenuTree = (menuItems, parent = null) => {
			return menuItems.reduce((result, menuItem) => {
				result.push({
					menuId: menuItem.menuId,
					menuName: menuItem.menuName,
					parentId: parent ? parent.menuId : null,
				...
				if (menuItem.children) {
					result.push(
						...flattenMenuTree(menuItem.children, menuItem)
					);
				}
		...
		const menuTree = this.menuListService.convertTree(response1.data);
		const updatedMenuList = mapFeatureNamesToMenuItems(
			menuTree,
			response4.data
		);
		const flatMenuList = flattenMenuTree(updatedMenuList);

		this.setState(
			{
				rawData: flatMenuList,
				data: updatedMenuList,
				selectedMenu: updatedMenuList,
				role: response2.data,
				ActionItems: this.getCheckedItems(
					this.removeEmptyChildren(updatedMenuList)
				),
				currentuser: response3.data,
				feature: response4.data,
			},
			() => {
				if (this.props.params.id !== null) {
					const filterRoleName = this.state.role.find(
						(roleItem) =>
							roleItem.roleId === parseInt(this.props.params.id)
					);
					if (filterRoleName) {
						this.onSelectRole(filterRoleName.roleName);
					}
				}
				this.shortCommonFeatures(updatedMenuList);
			}
		);
	...
	onSelectRole = (value) => {
		...
		this.userAccessService.retrieveByRoleId(roleId).then((response) => {
			...
			const updatedData =
				this.updateDataWithUserAccessData(userAccessData);
			...
			case this.state.selectedMenu.length < this.state.data.length &&
				...
				this.setState(
					...
					() => {
						...
						const ActionItems =
							this.handleActionItemsOnSelectRole(
								updatedCommonFeature
							);
	...
	updateFeatureStatus = (
		...
		if (
			...
			updatedUserAccessFeatureMappings[
				featureIndex
			].userAccessFeature.status = isChecked;
			this.setState(
				(prevState) => ({
	...
	shortCommonFeatures = (data, commonFeatureData = {}) => {
		data.forEach((item) => {
			if (item.parentId === null) {
				...
				filterFeatures.sort();
				const uniqueCommonFeature = filterFeatures.map(
					(featureName) => {
	...
	handleCommonFeature = (checked, itemKey, featureName) => {
		this.setState((prevState) => {
			const updatedFeatureData = prevState.commonFeature[itemKey].map(
				(item) =>
			...
			if (featureName.toLowerCase() === "view" && checked === false) {
				...
				updatedCommonFeature[itemKey] = updatedCommonFeature[
					itemKey
				].map((item) => ({
					...item,
					status: false,
			...
			if (Array.isArray(data)) {
				data.forEach(({ userAccessFeatureMappings, children }) => {
					if (
						...
						userAccessFeatureMappings.forEach(
							({ userAccessFeature }) => {
	...
	getIndeterminate = (key) => {
		...
		if (!Array.isArray(commonFeatureArray)) {
			return false; // or handle the case appropriately
		}

		const unCheckAll = commonFeatureArray.every(
			(item) => item.status === false
		);
		const checkAll = commonFeatureArray.every(
			(item) => item.status === true
		);
		let indeterminate = commonFeatureArray.some(
			(item) => item.status === true
		);
	...
	render() {
				...
				render: (userAccessFeatureMappings, record, index) => {
					if (record.parentId !== null) {
						const uniqueFeatures = [
							...new Set(
								(Array.isArray(userAccessFeatureMappings)
									? userAccessFeatureMappings
									: []
								).flatMap(
						...
						return (
							<>
								{Object.keys(this.state.commonFeature).map(
									(key) => {
										if (record.key === parseInt(key)) {
											return this.state.commonFeature[
												key
								...
								>
									...
									>
										<Checkbox
											...
											checked={
												this.state.ActionItems[menu.key]
											}
											disabled={
		...
		const roleName = localStorage.getItem("uRoleName");

		return (
			<Spin spinning={this.props.isLoading}>
				{userName === "Administrator" || roleName === "Super User" ? (
					<Page title='User Access'>
						...
						<Form onFinish={this.handleSubmit}>
							...
							>
								<Table
									columns={columns}
									dataSource={this.state.selectedMenu}
								/>
