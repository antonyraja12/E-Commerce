          ...
          <Row gutter={[10, 10]}>
                  ...
                  <Col md={21}>
                    <Descriptions bordered>
                      ...
                      <Descriptions.Item label="Latitude">
                        {this.state.asset?.latitude}
                      </Descriptions.Item>
                      <Descriptions.Item label="Longitude">
                        {this.state.asset?.longitude}
                      </Descriptions.Item>
                      <Descriptions.Item label="Total Parameters">
                        {this.state.asset?.parameters.length}
                      </Descriptions.Item>
                      <Descriptions.Item label="Total Alerts">
                        {this.state.asset?.alerts.length}
                      </Descriptions.Item>
                    </Descriptions>
                  </Col>
                </Row>
            ...
            <Col sm={24}>
              <Card
                ...
                extra={
                  <>
                    {this.state.parameters?.length > 6 && (
                      <Button
                        size="small"
                        type="primary"
                        onClick={this.tileMore}
                      >
                        {this.state.tileMore ? "Hide" : "Show More"}
                      </Button>
                    )}
                  </>
                }
              >
                {!this.state.tileMore ? (
                  <Row gutter={[10, 10]} className="parameterList">
                    {this.state.parameters?.slice(0, 6).map((e) => (
                      <Col xs={24} sm={12} md={8} lg={4}>
                        {/* <HomeCard
                            title={e.displayName}
                            value={e.parameterValue}
                            key={`parameter${e.parameterId}`}
                          ></HomeCard> */}
                      </Col>
                    ))}
                  </Row>
              ...
              <Row gutter={[10, 10]}>
                <Col sm={24}>
                  <Card
                    ...
                    actions={[
                      <>
                        {this.state.widget?.length < 3 && (
                          <Button type="ghost" onClick={this.addWidget}>
                            Add Widget
                          </Button>
                        )}
                      </>,
