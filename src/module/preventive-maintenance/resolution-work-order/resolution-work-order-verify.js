...
class ResolutionWorkOrderVerify extends PageForm {
  ...
  columns = [
    ...
    //     return `${value?.sparePartName} - #${value?.sparePartNumber}`;
    //   },
    // },
    // {
    //   title: "Quantity",
  ...
  expandedRowRender = (record) => {
    const columns = [
      {
        ...
        dataIndex: ["sparePart", "sparePartName"],
        key: "sparePartName",
      },
      {
        title: "Quantity",
      ...
      {
        title: "Condition",
        dataIndex: ["sparePart", "newSpare"],
        render: (value) => {
          console.log("new spare", value);
          return value ? "New" : "Refurbished";
        },
      },
    ];
    ...
    return (
      <Table
        columns={columns}
        dataSource={record.spareRequestSubList}
        rowKey="spareRequestSubId"
        pagination={false}
      />
    );
  };
          ...
          <Col span={24}>
            <div style={{ float: "right" }}>
              <Tooltip placement="top" title="Knowledge Base">
                <Link
                  to={`https://maxbyte-poc.document360.io/v1/en`}
                  target="_blank"
...
export default withAuthorization(withForm(ResolutionWorkOrderVerify));
